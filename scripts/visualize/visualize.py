...
from models.image_transforms import ImageTransform
...
def visualize_feature_matches(feature_matches: FeatureMatches):
    ...
    """
    image_1 = feature_matches.image_and_features_1.image
    image_2 = feature_matches.image_and_features_2.image
...
def visualize_image_transforms(feature_matches: FeatureMatches, image_transform: ImageTransform):
    """
    Displays a pair of images with overlays for each feature match and features in image 1 transformed to image 2
    ...
    feature_matches:
        A Matches object to be visualized
    image_transform:
        A transform of coordinates in image 1 to image 2
    """
    image_1 = feature_matches.image_and_features_1.image
    image_2 = feature_matches.image_and_features_2.image
    if image_1.shape != image_2.shape:
    ...
    features_1_xformed = ImageTransform.apply_transform(feature_matches.image_and_features_1.to_points(), image_transform)

    for match in feature_matches.matches:
        index_1, index_2 = match
        feature_1 = features_1[index_1]
        bounding_box_1 = feature_to_bounding_box(feature_1)
        draw.ellipse(xy=bounding_box_1, fill=None, outline='yellow')

        feature_2 = features_2[index_2]
        x0, y0, x1, y1 = feature_to_bounding_box(feature_2)
        bounding_box_2 = x0 + image_1_cols, y0, x1 + image_1_cols, y1
        draw.ellipse(xy=bounding_box_2, fill=None, outline='yellow')

        x_trans, y_trans = features_1_xformed[index_2, :]
        x0, y0, x1, y1 = feature_to_bounding_box(Feature(x=x_trans, y=y_trans))
        bounding_box_2 = x0 + image_1_cols, y0, x1 + image_1_cols, y1
        draw.ellipse(xy=bounding_box_2, fill=None, outline='magenta')

        draw.line(xy=(feature_1.x, feature_1.y, x_trans + image_1_cols, y_trans), fill='blue')
    image_pair.show()
